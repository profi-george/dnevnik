...
export default async function ProjectsPage() {
  ...
  const states = await getProjectStates(projects.map((p) => p.id));
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">
      ...
      <ul className="card divide-y divide-line-soft">
        {projects.map((project) => {
          ...
          // Вторая строка — не просто счётчик, а что с проектом сейчас: просрочки
          // важнее даты последней правки, следующий шаг плана — запасной вариант,
          // если просрочек нет вовсе.
          const secondLine = state
            ? [
                state.lastActivity && formatDateRu(state.lastActivity),
                state.overdueCount > 0
                  ? `${state.overdueCount} просрочено`
                  : state.nextAction
                    ? `следующее: ${state.nextAction}`
                    : null,
              ]
                .filter(Boolean)
                .join(" · ")
          ...
          return (
            <li key={project.id} className="group flex items-center gap-2 px-2.5 py-1.5">
              <Link
                href={`/projects/${project.id}`}
                className="flex min-w-0 flex-1 flex-col gap-0.5 rounded px-1 py-1 hover:bg-hover-soft"
              >
                ...
                {secondLine && (
                  <span
                    className={`truncate text-2xs ${state && state.overdueCount > 0 ? "text-danger" : "text-fg-subtle"}`}
                  >
                    {secondLine}
                  </span>
                )}
